...
@section('content')
                    ...
                    <div class="col-lg-8 col-lg-offset-2">
                        <section class="panel">
                            ...
                            <div class="panel-body">
                                <form role="form" action="{{ route('grop.update', ['grop' => $grop->id]) }}"
                                      method="POST"
                                      enctype="multipart/form-data" autocomplete="off">
                                    @csrf
                                    @method('put')
                                    <div class="form-group">
                                        <label for="exampleInputPassword1">نام دسته بندی</label>
                                        <input type="text" name="name" value="{{ $grop->name }}"class="form-control" id="exampleInputPassword1">
                                    </div>
                                    <div class="form-group">
                                        <label for="exampleInputPassword1">ایکون</label>
                                        <select style="font-family: fontAwesome" class="form-control" name="icon" id="">
                                    ...
                                    <div style="display: flex;justify-content: space-around;flex-wrap: wrap">
                                        ...
                                        <input type="hidden" id="attrbute" name="sub" value="{{ json_encode($grop->subGrop) }}">
                                        <button style="margin-top: 12px;" type="submit" name="btn" class="addSub btn btn-info">اضافه مشخصه </button>
                                    </div>
                                    <table class="table">
                                        <thead>
                                        <tr>
                                        ...
                                        <tbody class="subTbl" id="table">
                                        @foreach ($grop->subGrop as $item)
                                            <tr>
                                                <td>{{ $item }}</td>
                                                <td style='border-radius:12px;background:red;padding:12px;color:#ffff;'
                                                    class="deleteTr" onclick="deltSub(event)" class="deleteTr">delet
                                                </td>
                                            </tr>
                                        @endforeach
                                        </tbody>
    ...
    <script>
        arraySub = JSON.parse($('#attrbute').val());
        ...
        $('.addSub').click(function (e) {
            ...
            if ($('#key').val() != '') {
                ...
                <tr>
                    <td>${$('#key').val()}</td>
                    <td style='border-radius:12px;background:red;padding:12px;color:#ffff;' onclick='deltSub(event)'>delet</td>
                </tr>
            `);
                arraySub.push($('#key').val());
                $('#key').val('');
                console.log(arraySub);
                $('#attrbute').val(JSON.stringify(arraySub));
            }
        });
        let deltSub = (event) => {
            let index = $(event.target).prev().text();
            let subs = JSON.parse($('#attrbute').val())
            subs.splice(subs.indexOf(index), 1);
            arraySub.splice(subs.indexOf(index), 1);
            console.log(subs)
